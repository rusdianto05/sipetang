...
<div class="col-12 grid-margin stretch-card">
    <div class="card">
      <div class="card-body">
        <h4 class="card-title">Edit Surat Keterangan Jalan</h4>
        <p class="card-description">
          Edit form dibawah ini
        </p>
        <form class="forms-sample" method="POST" action="{{ url('/keterangan/jalan/'.$item->id) }}" enctype="multipart/form-data">
          @csrf
          @method('PUT')
          @if($errors->any())
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-control" id="valid_from" name="valid_from" value="{{ $item->valid_from }}">
            @error('valid_from')
              <div class="alert alert-danger">{{ $message }}</div>
              @enderror
          </div>
          <div class="form-group">
            ...
            <input type="text" class="form-control" id="valid_until" name="valid_until" value="{{ $item->valid_until }}">
            @error('valid_until')
              <div class="alert alert-danger">{{ $message }}</div>
              @enderror
          </div>
