@extends('layouts.app')
@section('title', 'Keterangan Wali')
@section('content')
<div class="col-12 grid-margin stretch-card">
    <div class="card">
      <div class="card-body">
        <h4 class="card-title">Edit Surat Keterangan Wali</h4>
        <p class="card-description">
          Edit form dibawah ini
        </p>
        <form class="forms-sample" method="POST" action="{{ url('/keterangan/wali/'.$item->id) }}" enctype="multipart/form-data">
          @csrf
          @method('PUT')
          @if($errors->any())
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-control" id="regency" name="regency" value="{{ $item->regency }}" placeholder="regency">
          </div>
          <div class="form-group">
            <label for="sub_district">Kecamatan</label>
            <input type="text" class="form-control" id="sub_district" name="sub_district" value="{{ $item->sub_district }}" placeholder="sub_district">
          </div>
          <div class="form-group">
            <label for="village">Desa</label>
            <input type="text" class="form-control" id="village" name="village" value="{{ $item->village }}" placeholder="village">
          </div>
       @role('admin')
          <div class="form-group">
